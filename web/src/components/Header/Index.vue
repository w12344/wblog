            ...
            <div class="top-bar-right pull-right text-right">
              <div class="top-admin">
                <div class="nav-item" v-for="(navItem, navIndex) in navList" :key="navIndex" @click="handleNavItemChange(navItem)">
                  ...
                  <el-popover v-if="navItem.key === 'username'" placement="bottom" width="100" trigger="click">
                    <ul class="popbox">
                      <li>个人中心</li>
                      <li>个人设置</li>
                      <li>退出登录</li>
                    </ul>
                    <span class="nav-item-text" slot="reference">{{ username }}</span>
                  </el-popover>
                  <span class="nav-item-text" v-else>{{ navItem.navName }}</span>
                </div>
...
import { NavParams, NavListItem, loginData } from '../../const/home';
...
export default class Header extends Vue {
  @State((state) => state.userStore.userinfo) userinfo!: loginData;
  ...
  private navList: Array<NavListItem> = [
    { key: 'ranking', navName: '排行榜', icon: 's-data', path: '' },
    { key: 'total', navName: '统计', icon: 'notebook-2', path: '' },
    { key: 'username', navName: '登录', icon: 'user', path: '/login' },
  ];
